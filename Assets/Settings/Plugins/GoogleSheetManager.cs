...
public class GoogleSheetsManager : MonoBehaviour
{
    ...
    public async void GoogleStart()
    {
        ...
        //await LoadBuildingDataFromGoogleSheets();
    }
    ...
    //public List<QuestRequiredItem> GetRequiredItemsForQuest(int questId)
    //{
    //    return requiredItemsList.FindAll(item => item.QuestID == questId);
    //}

    /*    Enum GetSubType(ProductType productType, string subTypeName)
        {
            switch (productType)
            {
                case ProductType.MeatFish:
                    return (MeatFishType)Enum.Parse(typeof(MeatFishType), subTypeName);
                case ProductType.VegetableFruit:
                    return (VegetableFruitType)Enum.Parse(typeof(VegetableFruitType), subTypeName);
                case ProductType.ProcessedFood:
                    return (ProcessedFoodType)Enum.Parse(typeof(ProcessedFoodType), subTypeName);
                case ProductType.GeneralGoods:
                    return (GeneralGoodsType)Enum.Parse(typeof(GeneralGoodsType), subTypeName);
                default:
