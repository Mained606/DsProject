...
// 플레이어 데이터
[Serializable]
public class PlayerData : CharacterData, ICloneable
{
    ...
    {
        var clone = (PlayerData)base.Clone();
...
// 몬스터 데이터
[Serializable]
public class MonsterData : CharacterData, ICloneable
{
    public List<int> dropItems = new List<int>(); // 드롭 아이템
    public int experienceReward; // 경험치 보상
    public int goldReward;       // 골드 보상

    public MonsterData(string name, MonsterType monsterType, GameObject prefab, int strength, int vitality, int agility,
    ...
    public new object Clone()
    {
        var clone = (MonsterData)base.Clone();
        clone.dropItems = new List<int>(this.dropItems);
